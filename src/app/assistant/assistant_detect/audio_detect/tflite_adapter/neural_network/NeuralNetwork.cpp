...
NeuralNetwork::NeuralNetwork(const void *model, size_t arenaSize)
{
    m_error_reporter = new tflite::MicroErrorReporter();
    m_tensor_arena = (uint8_t *)ps_malloc(arenaSize);
    if (!m_tensor_arena)
    {
    ...
    m_interpreter = new tflite::MicroInterpreter(
        m_model, *m_resolver, m_tensor_arena, arenaSize, m_error_reporter);

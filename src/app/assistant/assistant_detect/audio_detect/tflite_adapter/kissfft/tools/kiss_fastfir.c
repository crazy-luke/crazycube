        ...
        void * mem,size_t*lenmem)
{
    ...
    if (lenmem == NULL) {
        st = (kiss_fastfir_cfg) KISS_FFT_MALLOC (memneeded);
    } else {
        if (*lenmem >= memneeded)
        ...
        size_t n_imp_resp)
{
    ...
    nbuf = 4096;
    buf = (kffsamp_t *) KISS_FFT_MALLOC ( sizeof (kffsamp_t) * nbuf);
    circbuf = (kffsamp_t *) KISS_FFT_MALLOC (sizeof (kffsamp_t) * nlag);
    if (!circbuf || !buf) {
        ...
        size_t nfft )
{
    ...
    inbuf = (kffsamp_t*)KISS_FFT_MALLOC(sizeof(kffsamp_t)*n_samps_buf);
    outbuf = (kffsamp_t*)KISS_FFT_MALLOC(sizeof(kffsamp_t)*n_samps_buf);
...
int main(int argc,char**argv)
{
    ...
    h = (kffsamp_t*)KISS_FFT_MALLOC(sizeof(kffsamp_t)*nh);
